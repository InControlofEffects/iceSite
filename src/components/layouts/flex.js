...
function FlexWrapper(props) {
    ...
    for (let i = 0; i < elements.length; i++) {
        elements[i] = React.cloneElement(elements[i], {
            className: elements[i].props.className ? `flex-child ${elements[i].props.className}` : `flex-child`
        })
    }
    return (
        <div
            id={props.id ? props.id : null}
            className={`flex flex-${layout}-layout 
                ${props.wrapReverse ? "flex-wrap-reverse" : "flex-wrap-default"} 
                ${props.className ? props.className : "flex-default"}
                `}
